<template>
  <v-app>
    ...
    <div class="full-image-wrap" v-if="fullImage">
      ...
      <!-- <v-col cols="12" sm="3">
          <v-btn class="fav-btn" icon color="pink">
            <v-icon>mdi-heart</v-icon>
          </v-btn>
        </v-col>
        <v-btn class="btn" @click="dispDown">CLOSE</v-btn>
        <v-btn v-on:click="check(currentImageUrl.split('/')[2])" class="btn2"
          >DELETE</v-btn
        > -->
    </div>
    <v-main class="v-main">
...
export default {
  ...
  methods: {
    ...
    check: async function (n) {
      ...
      if (result) {
        var param = n;
        this.delpath = await axios.post(`${endpoint}/summer/delete`, param);
    ...
    receiveChildMessage(e) {
      ...
      for (let i = 0; i < this.path.length; i++) {
        ...
        let fixPath = require("../../../backend/summer_files/" + element[2]);
        // console.log(fixPath);
  ...
  async mounted() {
    this.path = await axios.get(`${endpoint}/summer/get`);
