...
export default {
  ...
  methods: {
    ...
    check: async function (n) {
      ...
      if (result) {
        var param = n;
        this.delpath = await axios.post(`${endpoint}/winter/delete`, param);
    ...
    receiveChildMessage(e) {
      ...
      for (let i = 0; i < this.path.length; i++) {
        ...
        let fixPath = require("../../../backend/winter_files/" + element[2]);
        // console.log(fixPath);
  ...
  async mounted() {
    this.path = await axios.get(`${endpoint}/winter/get`);
